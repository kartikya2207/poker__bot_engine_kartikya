...
class RoundRobinTournament:
    def __init__(self, bots, repeats=1):
        self.bots = bots
        self.repeats = repeats
        self.global_scores = {name: 0 for name, _ in bots}

    def run(self):
        print("\n========== STARTING ROUND ROBIN TOURNAMENT ==========\n")

        for cycle in range(1, self.repeats + 1):
            print(f"\n===== Round Robin Cycle {cycle}/{self.repeats} =====\n")

            # Generate all unique pairs
            for (name1, file1), (name2, file2) in itertools.combinations(self.bots, 2):

                print(f"\n--- Match: {name1} vs {name2} ---\n")

                # Temporarily override globals
                global BOT_1_NAME, BOT_1_FILE
                global BOT_2_NAME, BOT_2_FILE

                BOT_1_NAME, BOT_1_FILE = name1, file1
                BOT_2_NAME, BOT_2_FILE = name2, file2

                match = PokerMatch(small_log=True)
                match.run()

                # Collect results
                # IMPORTANT: players are reversed internally per round,
                # so use final bankroll stored in BotProcess

                for line in match.log:
                    if line.startswith("Final"):
                        # parse final bankroll line
                        break

                # Extract final bankrolls directly
                # safer approach: modify PokerMatch.run() to return players

                # Instead we patch PokerMatch to store final players
                p1, p2 = match.final_players
                self.global_scores[p1.name] += p1.bankroll
                self.global_scores[p2.name] += p2.bankroll
        ...
        print(f"\n🏆 Overall Winner: {winner[0]} with score {winner[1]}")
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--small_log', action='store_true')
    args = parser.parse_args()

    from config import BOTS, ROUND_ROBIN_REPEATS

    tournament = RoundRobinTournament(BOTS, repeats=ROUND_ROBIN_REPEATS)
    tournament.run()